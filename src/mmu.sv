module mmu (clk, rst_n, flush, sel, mmu_in, mmu_w, mmu_bias, mmu_out);
    ...
    generate
        ...
        for (i=0; i<12; i++) begin: Buff
            for (j=0; j<7; j++) begin
                ff buff (.clk(clk), .rst_n(rst_n), .flush(0), .d(pe_out[i][j]), .q(buff_out[i][j]));
                ff fb_buff (.clk(clk), .rst_n(rst_n), .flush(flush), .d(acc_res[i][j]), .q(fb_buff_out[i][j]));
                assign acc_res[i][j] = buff_out[i][j] + fb_buff_out[i][j];
            end
        end
        for (i=0; i<7; i++) begin
            for (j=0; j<12; j++) begin
                assign tree_in[i][j] = sel? buff_out[i][j] : fb_buff_out[j][i];
            end
