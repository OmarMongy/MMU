module mmu_top (clk, rst_n, mmu_in, mmu_w, mmu_bias, mmu_out);
    ...
    generate
        ...
        for (i=0; i<12; i++) begin: Buff
            for (j=0; j<7; j++) begin
                ff buff (.clk(clk), .rst_n(rst_n), .d(pe_out[i][j]), .q(buff_out[i][j]));
                ff fb_buff (.clk(clk), .rst_n(rst_n), .d(acc_res[i][j]), .q(fb_buff_out[i][j]));
